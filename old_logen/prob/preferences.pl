...
%   print('% '),
   ...
   preferences:preference_default_value(P,_),
   call(preferences:preference(P,Val)),
   call(preferences:preference_val_type(P,T)),
   call(preferences:preference_category(P,_)),
   call(preferences:is_of_type(Val,T,_)),
   fail.
...
get_recent_documents(TclRes) :-
   findall(X,recent_documents(X),List),
   reverse(List,Res),
   convert_list_into_tclstring(Res,TclRes).
   

convert_list_into_tclstring([],'').
convert_list_into_tclstring([H|T],Res) :-
  convert_list_into_tclstring(T,TRes),
  atom_concat('} ',TRes,I1),
  atom_concat(H,I1,I2),
  atom_concat('{',I2,Res).
